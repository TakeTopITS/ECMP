...
public partial class TTCreatedProject : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Page.IsPostBack != true)
        {
            ...
            //    T_ProjectCostManage where Type='操作' group by ProjectID) B on A.ProjectID=B.ProjectID) D on C.ProjectID=D.ProjectID where C.UserCode='{0}' and 
            //    C.Status not in ('新建','隐藏','删除','归档') Order by C.ProjectID DESC", strUserCode); 
            ...
            strHQL = string.Format(@"WITH Cost AS (
                    ...
                    WHERE C.UserCode = '{0}'
                    AND C.Status NOT IN ('新建', '隐藏', '删除', '归档')
